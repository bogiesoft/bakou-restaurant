...
class Item extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('category_id, supplier_id, unit_id, allow_alt_description, is_serialized, deleted, topping', 'numerical', 'integerOnly'=>true),
			...
			array('id, name, item_number,unit_id, category_id, supplier_id, cost_price, unit_price, quantity, reorder_level, location, allow_alt_description, is_serialized, description, deleted, promo_price', 'safe', 'on'=>'search'),
		);
	}
	...
	public function attributeLabels()
	{
		return array(
			'id' => 'ID',
			'name' => Yii::t('model','model.item.name'),
			'item_number' => Yii::t('model','model.item.item_number'),
                        'unit_id' => Yii::t('model','model.item.unit_id'),
			'category_id' => Yii::t('model','model.item.category_id'),
			'supplier_id' => Yii::t('model','model.item.supplier_id'),
			'cost_price' => Yii::t('model','model.item.cost_price'),
			'unit_price' => Yii::t('model','Price 1'),
			'quantity' => Yii::t('model','model.item.quantity'),
			'reorder_level' => Yii::t('model','model.item.reorder_level'), 
			'location' => Yii::t('model','model.item.location'),  
			'allow_alt_description' => Yii::t('model','model.item.allow_alt_description'),
			'is_serialized' => Yii::t('model','model.item.is_serialized'),
			'description' => Yii::t('model','model.item.description'),
			'deleted' => Yii::t('model','model.item.deleted'),
                        'items_add_minus' => Yii::t('model','model.item.items_add_minus'), 
                        'inv_quantity' => Yii::t('model','model.item.inv_quantity'), 
                        'inv_comment' => Yii::t('model','model.item.inv_comment'),
                        'inventory' => Yii::t('model','model.item.inventory'),
                        'category_id' => Yii::t('model','model.item.category'),
                        'sub_quantity' => Yii::t('model','Sub Quantity'),
                        'promo_price' => Yii::t('model','Promo Price'),
                        'promo_start_date' => Yii::t('model','Promo Start'),
                        'promo_end_date' => Yii::t('model','Promo End'),
                        'topping' => Yii::t('model','Topping'),
		);
	}
         ...
         public function topping($category_id)
	{
        ...
        public static function getItem($name = '',$price_tier_id) {
            ...
            return Yii::app()->db->createCommand($sql)->queryAll(true, array(':item_name' => $item_name, ':item_number' => $item_number,':price_tier_id'=>$price_tier_id));
        }
        ...
        public function getItemPriceTier($item_id,$price_tier_id)
        {
           ...
           $result=Yii::app()->db->createCommand($sql)->queryAll(true, array(':item_id' =>(int)$item_id,':price_tier_id'=>$price_tier_id));         
           return $result;
        }
	...
	public function suggest($keyword,$limit=20)
	{
		/*
			...
			'condition'=>'(name LIKE :keyword or item_number like :item_number)',
			'order'=>'name',
			'limit'=>$limit,
			'params'=>array(':keyword'=>"%$keyword%",':item_number'=>"%$keyword%")
		));
		...
		$suggest=array();
			...
			$suggest[] = array(
				'label'=>$model->item_number .' - ' . $model->name.' - '. $model->unit_price ,  // label for dropdown list
            ...
            $models = Yii::app()->db->createCommand($sql)->queryAll(true, array(':item_name' => $item_name, ':item_number' => $item_number,':price_tier_id'=>Yii::app()->orderingCart->getPriceTier(),'zero'=>$this::_zero));
